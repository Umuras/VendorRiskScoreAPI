...
namespace VendorRiskScoreAPI.Repositories
{
    public class VendorProfileRepository : IVendorProfileRepository
    {
        ...
        {
            return await _context.VendorProfiles.Include(v => v.Document).Include(v => v.SecurityCerts).Include(v => v.RiskAssessment).Include(v => v.VendorProfileRiskScore).ToListAsync();
        }

        public async Task<VendorProfile> GetVendorProfileByIdAsync(int id)
        {
            VendorProfile? vendorProfile = await _context.VendorProfiles.Include(v => v.Document).Include(v => v.SecurityCerts).Include(v => v.RiskAssessment).Include(v => v.VendorProfileRiskScore).FirstOrDefaultAsync(v => v.Id == id);
